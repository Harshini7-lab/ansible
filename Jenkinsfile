pipeline {
  ...
  parameters {
    string(
      name: 'WEB_REPO_URL',
      defaultValue: 'https://github.com/Harshini7-lab/website.git',
    ...
    string(
      ...
      defaultValue: 'https://github.com/Harshini7-lab/ansible.git',
  ...
  stages {
    ...
    stage('Run Ansible (Apache + Deploy)') {
      steps {
        withCredentials([sshUserPrivateKey(credentialsId: 'ssh-id', keyFileVariable: 'PK')]) {
          sh '''#!/usr/bin/env bash
set -Eeuo pipefail
